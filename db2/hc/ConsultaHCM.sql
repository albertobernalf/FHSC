-- Primero conseguir los folios del paciente a listar de acuerdo supongo a un rango de fechas
...
-- Cabezote . Datos del paciente debe ir impreso al comienzo de cada pagina
-- Verificar de acuerdo a cual criterio se toman estos datos, si por el ingreso o por el numero de folio
-- Tambien como se va a controlar el CONTRATO del paciente
...
dis.discdsc discapacidad,pob.GruPobDes grupo_poblacional, concat(concat(concat(concat(concat(concat(i.ingnmresp ,' '),i.ingnmresp2),' '), i.ingapres),' '),i.ingapres2)  as responsable,  i.ingtelresp    as telefono,case when i.ingparresp='H' then'HIJO' when i.ingparresp='P' then'Padre'  when i.ingparresp='F' then 'Familiar'  end     as parentesco,i.ingnoac as acompanante,  i.ingteac as  telefono_acompanante 
...
where cap.mptdoc ='CC' and cap.mpcedu ='21075328';
...
where des1.histipdoc='CC' and des1.hisckey='21075328' and des1.hiscsec=2 and h1.fhcindesp IN ('GN')
union
select h1.hiscsec as folio,h1.hisfhorat, case when h1.hisclpr='2' then 'HOSPITALIZACION' when h1.hisclpr='1' then 'AMBULATORIO' when h1.hisclpr='3' then 'URGENCIAS' end tipo_atencion,
...
where h1.histipdoc='CC' and h1.hisckey='21075328' and h1.hiscsec=2 and h1.fhcindesp IN ('GN');

-- Diagnosticos :

SELECT * FROM HOSVITAL.HCDIAGN WHERE hisckey='79338503' and hiscsec =766;

select  hc.histipdoc,hc.hisckey,hc.hiscsec as folio, hc.hcdxcod, m.dmnomb, case when hc.hcdxcls=2 then 'DESCARTADO'  when hc.hcdxcls=1 then 'PRINCIPAL' when hc.hcdxcls=0 then 'RELACIONADO'  END tipo
...
h33.evodes,'Nota realizada por:' ,maemed1.mmnomm,concat(concat(cast(h33.evofec as varchar(10)),' '),h33.evohor)  fecha
from hosvital.hccom1 h1 
left join  hosvital.hccom33 h33 on (h33.histipdoc=h1.histipdoc and h33.hisckey=h1.hisckey and h33.hiscsec = h1.hiscsec )
...
left join hosvital.maemed1 maemed1 on (maemed1.mmusuario=h51.hcpruscad)
 where  h1.histipdoc='CC' and h1.hisckey='5039048'  and h1.hiscsec =250
order by pro.prcodi;
...
h5.hiscpcan cantidad,pro.prnomb as descripcion,  'Cancelado' as Cancelado, case when h51.hcmocntp='1' then 'Admistrativo' when h51.hcmocntp='3' then 'Medico'  end AS MOTIVO_CANCELACION,h51.hcobscan as observaciones,concat(concat(maemed1.mmnomm,'-RM: '), maemed1.mmregm) as usuario_cancela,h51.hcprfhcad as fecha_cancelacion, h5.hiscpobs
from hosvital.hccom1 h1
inner join hosvital.hccom5 h5 on (h5.histipdoc=h1.histipdoc and h5.hisckey=h1.hisckey  and h5.hiscsec=h1.hiscsec and h5.hcprctip= 3 and h5.hcprstgr='C')
inner join hosvital.hccom51 h51 on (h51.histipdoc= h5.histipdoc and h51.hisckey=h5.hisckey and h51.hiscsec=h5.hiscsec and h51.hcprccod= h5.hcprccod)
inner join hosvital.maepro pro on (pro.prcodi=h51.hcprccod)
left  join hosvital.maemed1 maemed1 on (maemed1.mmusuario=h51.hcpruscad)
 where  h1.histipdoc='CC' and h1.hisckey='21075328'  and h1.hiscsec =117
