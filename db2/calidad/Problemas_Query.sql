...
-- El mismo query pero al derecho
...
(select case when  mae2.hiscamfec > '2019-11-30'  then '2019-11-30 23:59:59' when  mae2.hiscamfec >= '2019-11-01' and mae2.hiscamfec <= '2019-11-30'  then cast(mae2.hiscamfec as varchar(12))||' '||mae2.hiscamhor else
'2019-11-30 23:59:59'  end
 from hosvital.maepab11 mae2
 where mae2.mptdoc=mae.mptdoc and mae2.mpcedu=mae.mpcedu and mae2.mpcodp=mae.mpcodp and mae2.mpnumc=mae.mpnumc  and mae2.hiscamfec >='2019-11-01' and mae2.hiscamedo='L'  and mae2.hiscnsing = mae.hiscnsing  --and  --mae2.hiscamctv> mae.hiscamctv 
 and mae2.hiscamctv = (select min(x.hiscamctv)
							           from hosvital.maepab11 x
							          where x.mptdoc=mae2.mptdoc and x.mpcedu=mae2.mpcedu and x.mpcodp=mae2.mpcodp and x.mpnumc=mae2.mpnumc and x.hiscnsing = mae2.hiscnsing and  x.hiscamedo='L' and x.hiscamfec >='2019-11-01' ) -- and x.hiscamctv  >  mae.hiscamctv )
...
where  i.mpcedu= '5770765'  and (i.ingfecadm<= '2019-11-30 23:59:59' )   -- and i.mpcodp = m.mpcodp 
 and mae.hiscamedo ='O'   -- and  mae.hiscamfec  >= '2019-11-01'
 and mae.hiscamfec <= '2019-11-30' and (mae.mptdoc,mae.mpcedu,mae.hiscnsing, mae.mpnumc) in
(select y.mptdoc,y.mpcedu,y.hiscnsing, y.mpnumc from hosvital.maepab11 y where y.hiscamfec >='2019-11-01' )
order by 1;

-- 5770765 no tien fecha de lib
-- 20277770  muchosm registros
-- 2657245  tiene dos registros de ocupacion
-- 93348510 muchos problemas

select * from hosvital.maepab11 where mpcedu = '5770765' order by hiscamfec;
