...
-- 1|110010413101|NI|860007373|2019-07-01|2019-12-31|total_registros
...
where i.citfecpa >= '2019-07-01'  and i.citfecpa <= '2019-12-31' and ((i2.citpro >='881112'  and  i2.citpro <='882841' )or  (i2.citpro >= '883101' and i2.citpro<='883910'  ))
and
...
inner join hosvital.ctrlcitas c1 on (c1.citnum= i.citnum and c1.citstscit='R')
inner join hosvital.maeemp m on (m.mennit = i.citnrocto)
where   i.citfecpa >= '2019-07-01'  and i.citfecpa <= '2019-12-31' 

and i2.citpro in ('890201','89020202','890203','890266','89020205','89020213','89030213','890235','89020202')  and i2.citprivez='S'

  and concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(c1.citnum,year(c1.citfchhra)),'-'),month(c1.citfchhra)),'-'),day(c1.citfchhra)),' '),hour(c1.citfchhra)) ,':'),minute(c1.citfchhra)),':'),second(c1.citfchhra))           = (select min (concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(c1.citnum,year(x1.citfchhra)),'-'),month(x1.citfchhra)),'-'),day(x1.citfchhra)),' '),hour(x1.citfchhra)) ,':'),minute(x1.citfchhra)),':'),second(x1.citfchhra)))
...
where  p.profec >= '2019-07-01' and p.profec <= '2019-12-31' and  p1.crgcod >='010101' and p1.crgcod <= '869700'  ;
...
(select concat(concat(concat(concat(concat(concat(concat(concat(concat(year(h1.hisfhorat),'-'),varchar_format(h1.hisfhorat,'MM')),'-'),varchar_format(h1.hisfhorat,'DD')),'|'), varchar_format(h1.hisfhorat,'HH')),':'),varchar_format(h1.hisfhorat,'MI')),'|')
...
(select concat(concat(concat(concat(concat(concat(concat(concat(year(h1.hisfhorat),'-'),varchar_format(h1.hisfhorat,'MM')),'-'),varchar_format(h1.hisfhorat,'DD')),'|'), varchar_format(h1.hisfhorat,'HH')),':'),varchar_format(h1.hisfhorat,'MI'))
 from hosvital.hccom1 h1 
...
where  i.ingfecadm >= '2019-07-01 00:00:00' and i.ingfecadm  <= '2019-12-31 23:59:59' and i.mpcodp in (1,18,20,21,35,38,39,40)
 ;
...
-------------------------------------------
----------------------------------------------
/* ojo este caso se repite
select * from hosvital.citmed1 where citced= '79897486' ;  --  Este caso se repite
...
-- este paciente tambien 79897486
- este opaciente 51603507
*/