...
select * from movcont2 where doccod = 'NFR' AND MVCNRO  IN
( 12615, 12616 ,12617)
...
SELECT empcod ,  doccod ,  57990     ,  mcdpto ,  'Salto de consecutivo' ,  agccdg ,  tranapl ,  mvcfch ,
  ...
  mvsrcerr ,  mvcmayaux ,  mvcmaygen , mvccnvmon ,  mvcorigen ,NULL,   mvcestpre ,NULL,NULL,NULL
    FROM MOVCONT3
WHERE DOCCOD='TCE' AND MVCNRO=57991
-- COMMIT
-- ROLLBACK;

BEGIN TRANSACTION;
INSERT INTO MOVCONT2
SELECT empcod ,  doccod ,  57990     ,  mcdpto ,  mvccsc ,  cntvig ,  cntcod ,  cnucod ,  cnusub ,  cnccod ,  cntsub , trccod ,  57990     ,  '' ,
  '' ,  mvcnat ,  0 ,  mvcvlrex ,  'Salto en consecutivos' ,  mvcbse ,  0 ,  mvcimpcod ,  mvccfch ,  mvcsedorg ,  mvcmes ,  mvcanio ,  mvccmpaj ,  mvcdocrf4 ,NULL,NULL
FROM MOVCONT2
WHERE DOCCOD='TCE' AND MVCNRO=57991  AND MVCNAT='D' AND MVCCSC=2
-- COMMIT
-- ROLLBACK
...
SELECT empcod ,  doccod ,  57990     ,  mcdpto ,  mvccsc ,  cntvig ,  cntcod ,  cnucod ,  cnusub ,  cnccod ,  cntsub , trccod ,  57990     ,  '' ,
  '' ,  mvcnat ,  0 ,  mvcvlrex ,  'Salto en consecutivos' ,  mvcbse ,  0 ,  mvcimpcod ,  mvccfch ,  mvcsedorg ,  mvcmes ,  mvcanio ,  mvccmpaj ,  mvcdocrf4 ,NULL,NULL
FROM MOVCONT2
WHERE DOCCOD='TCE' AND MVCNRO=57991 AND MVCNAT='C' AND MVCCSC=1
-- COMMIT;
-- ROLLBACK
