...
where t1.cntvig='2019' and  t1.mcdpto='001'    --and t1.prvcod = '80251797' 
GROUP BY  t1.empcod,T1.CNTVIG,prvcod,trcrazsoc ,     t1.hoptppz   
having
(select SUM(hoptotcre-hoptotdeb) 
 from hosvital.hojoblprv x 
 where  x.empcod = t1.empcod and   x.prvcod= t1.prvcod and x.cntvig= t1.cntvig   and  cast( (case when x.hoptppz='' then 0 else x.hoptppz end)  as numeric) = cast( (case when t1.hoptppz='' then 0 else t1.hoptppz end)  as numeric)  ) > 0  
order by trcrazsoc